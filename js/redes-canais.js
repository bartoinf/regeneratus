...
function addToHistory(channels) { const history = getHistory(); history.unshift({ id: Date.now(), topic: topic.value.trim(), objective: objective.value, tone: tone.value, createdAt: new Date().toLocaleString("pt-BR"), channels }); saveHistory(history); }
function toggleFavorite(key, text, metadata = {}) { const favorites = getFavorites(); const index = favorites.findIndex(item => item.channel === key && item.text === text); if (index >= 0) favorites.splice(index, 1); else favorites.unshift({ id: Date.now(), channel: key, text, topic: metadata.topic || topic.value.trim(), objective: metadata.objective || objective.value, tone: metadata.tone || tone.value, createdAt: new Date().toLocaleString("pt-BR") }); saveFavorites(favorites); renderFavorites(); }
function renderChannelCard(key, text, historyId = null) { return `<article class="result-card channel-result" data-channel="${key}" data-history-id="${historyId || ""}"><h3>${channelNames[key]}</h3><textarea class="generated-text editable-channel" data-channel="${key}" aria-label="Editar conteúdo para ${channelNames[key]}">${escapeHtml(text)}</textarea><div class="result-actions"><button type="button" class="button save-channel" data-channel="${key}">Salvar edição</button><button type="button" class="button secondary-button copy-channel" data-channel="${key}">Copiar</button><button type="button" class="button secondary-button favorite-channel" data-channel="${key}">${isFavorite(key, text) ? "★ Favoritado" : "☆ Favoritar"}</button></div><p class="edit-hint">Edite esta versão sem alterar os outros canais.</p></article>`; }
function bindChannelActions() {
  document.querySelectorAll(".save-channel").forEach(button => button.addEventListener("click", function () { const key = this.dataset.channel; const textarea = document.querySelector(`.editable-channel[data-channel="${key}"]`); const card = this.closest(".channel-result"); if (!textarea || !textarea.value.trim()) return; const historyId = Number(card.dataset.historyId); const history = getHistory(); const entry = history.find(item => item.id === historyId); if (entry && entry.channels[key]) { entry.channels[key] = textarea.value.trim(); saveHistory(history); } this.textContent = "Edição salva!"; setTimeout(() => this.textContent = "Salvar edição", 1800); }));
  document.querySelectorAll(".copy-channel").forEach(button => button.addEventListener("click", async function () { const key = this.dataset.channel; const textarea = document.querySelector(`.editable-channel[data-channel="${key}"]`); if (!textarea) return; try { await navigator.clipboard.writeText(textarea.value); this.textContent = "Copiado!"; setTimeout(() => this.textContent = "Copiar", 1500); } catch (error) { alert("Não foi possível copiar o conteúdo."); } }));
  document.querySelectorAll(".favorite-channel").forEach(button => button.addEventListener("click", function () { const key = this.dataset.channel; const textarea = document.querySelector(`.editable-channel[data-channel="${key}"]`); if (!textarea || !textarea.value.trim()) return; toggleFavorite(key, textarea.value.trim(), { topic: topic.value.trim(), objective: objective.value, tone: tone.value }); this.textContent = isFavorite(key, textarea.value.trim()) ? "★ Favoritado" : "☆ Favoritar"; }));
}
function renderHistory() {
  const history = getHistory(); let container = document.getElementById("channelHistory");
  ...
  document.querySelectorAll(".channel-history-use").forEach(button => button.addEventListener("click", function () { const item = getHistory()[Number(this.dataset.index)]; if (!item) return; topic.value = item.topic; objective.value = item.objective; tone.value = item.tone; channel.value = Object.keys(item.channels).length === Object.keys(channelNames).length ? "todos" : Object.keys(item.channels)[0]; contentResult.innerHTML = `<div class="results-grid">${Object.entries(item.channels).map(([key, text]) => renderChannelCard(key, text, item.id)).join("")}</div>`; bindChannelActions(); window.scrollTo({ top: 0, behavior: "smooth" }); }));
...
function renderFavorites() {
  const favorites = getFavorites(); let container = document.getElementById("channelFavorites");
  ...
  document.querySelectorAll(".favorite-remove").forEach(button => button.addEventListener("click", function () { const item = getFavorites()[Number(this.dataset.index)]; if (item) toggleFavorite(item.channel, item.text, item); }));
...
generateContent.addEventListener("click", async () => {
  ...
  try {
    ...
    const channels = await window.RegeneratusGenerator.generate({ kind: "channels", channels: selectedChannels, objective: objective.value, tone: tone.value, topic: topicValue });
    const historyId = Date.now(); addToHistory(channels); const history = getHistory(); history[0].id = historyId; saveHistory(history);
  ...
  } catch (error) { contentResult.innerHTML = `<div class="result-card"><h3>Não foi possível gerar o conteúdo</h3><p>${escapeHtml(error.message)}</p></div>`; }
  finally { generateContent.disabled = false; generateContent.textContent = "Gerar conteúdo"; }
});
